...
# PLOT
pdf("Support_receiving_preds.pdf",width=8, height=3 )

p1 <- ggplot(NULL, aes(x=soc_nat[[4]], y=fsr_med ) )+
  geom_line(color=color3, lwd=line_width)+
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_ribbon( aes(ymin=esr_HPDI[1,], ymax=esr_HPDI[2,]), alpha=shade_alpha)+
  ylim(0, y2) +
...
p2 <- ggplot(NULL, aes(x=soc_nat[[9]], y=esr_med ) )+
...
p3 <- ggplot(NULL, aes(x=soc_nat[[14]], y=msr_med ) )+
