...
# PLOT
pdf("Grooming_receiving_preds.pdf",width=8, height=3 )

p1 <- ggplot(NULL, aes(x=soc_nat[[2]], y=fgr_med ) )+
  geom_line(color=color3, lwd=line_width)+
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_ribbon( aes(ymin=egr_HPDI[1,], ymax=egr_HPDI[2,]), alpha=shade_alpha)+
  ylim(0, y2) +
...
p2 <- ggplot(NULL, aes(x=soc_nat[[7]], y=egr_med ) )+
...
p3 <- ggplot(NULL, aes(x=soc_nat[[12]], y=mgr_med ) )+
